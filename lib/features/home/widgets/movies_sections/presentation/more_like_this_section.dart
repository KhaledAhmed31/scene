...
class _MoreLikeThisSectionState extends State<MoreLikeThisSection> {
  ...
  Widget build(BuildContext context) {
    MoreLikeThisCubit moreLikeThisCubit = getIt.get<MoreLikeThisCubit>();
    return BlocProvider.value(
      value:moreLikeThisCubit..fetchMoreLikeThis((widget.movieId).toString()),
      child: BlocConsumer<MoreLikeThisCubit, MoreLikeThisState>(
        ...
        builder: (context, state) {
          if (state is MoreLikeThisLoadingState) {
            ...
            List<SectionsEntity> movies =
                moreLikeThisCubit.moreLikeThisMovies ?? [];
            if (movies.isEmpty) {
              return Center(
                child: Text(
                  "NO MORE LIKE THIS",
